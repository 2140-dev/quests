...
For a set of coins $A = \set(Coin_{0}, .., Coin_{n})$, we define $Agg_{A} = H_{salt}(Coin_{0}) + .. + H_{salt}(Coin_{n})$.

Is it possible to show that this aggregate is provably secure against a probabilistic polynomial time algorithm? i.e. can an attacker create a set $C$, such that $Agg_{A} = Agg_{B} = Agg_{C}$ 
